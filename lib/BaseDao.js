 ...
 * @param callback 回调function(err, data)
 */
BaseDao.prototype.doQuery = function (conditions, order, callback, columns) {
 ...
 * @param callback 回调function(err, data)
 */
BaseDao.prototype.doQueryColumnForPage = function (conditions, onlyList, pageSize, pageNo, order, callback) {
    if(conditions == 'undefined') conditions = null;
    if(order == 'undefined' || order == null) order = 'id';
    if(pageSize == null) pageSize = 10 ; else pageSize = parseInt(pageSize);
    if(pageNo == null) pageNo = 0; else pageNo = parseInt(pageNo);

    BaseDBHelper.getPageByConditionsOnlyList(this.modelObj['tableName'], conditions, onlyList, pageSize, pageNo, order, function( err, rows) {
